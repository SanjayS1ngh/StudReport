...
void Update_Record(int roll, string name, string dept, int marks) {
	Node* p = head;
	while (p != NULL) {
		if (p->roll == roll) {
			p->Name = name;
...
int main()
{
	...
	while (true) {
		...
		if (Choice == 1) {
			...
			cout << "Enter Roll Number of Student whose record is to be updated\n";
			cin >> Roll;
			...
			cin >> Marks;
			Update_Record(Roll, Name, Course, Marks);
		}
		else if (Choice == 6) {
			exit(0);
		}
